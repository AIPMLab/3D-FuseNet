...
for num, optim in enumerate(p for p in module_path.iterdir() if p.is_dir()):
    ...
    for i in [rf, svrp, svrr, lr, knn]:
        ...
        kmf.fit(durations=y_test_ret,
                event_observed=[1 for i in y_test_ret],
                label="Truth")
        ax = kmf.plot_survival_function()
        kmf.fit(durations=y_pred_ret,
                event_observed=[1 for i in y_pred_ret],
                label="Predict")
        ...
        plt.savefig(module_path / rf"{optim.stem}_and_{nc[output[-1][1]]}.pdf",
                    bbox_inches='tight')
        pd.DataFrame({
            "y_true": y_test_ret,
            "y_pred": y_pred_ret
        }).to_csv(module_path / rf"{optim.stem}_and_{nc[output[-1][1]]}.csv")
#plt.savefig("2-t.png")
