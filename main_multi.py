...
from utils import SlidingWindowInference, EarlyStopping
import time
import rich
...
with console.status("Loading Dataset..."):
    ...
    test_dataloader = DataLoader(test_dataset,
                                ...
                                num_workers=0,
                                pin_memory=True)
rprint(f"Loading Dataset... [green]Complete[default], Using {time.time()-start_time:.2f} sec")
...
criterion_segment = losses.DiceLoss(to_onehot_y=False,
                                    sigmoid=True).cuda()
...
overall_progress = Progress(*Progress.get_default_columns(),
                            TimeElapsedColumn(), TextColumn("Best Epoch={task.fields[best_epoch]}, No Update={task.fields[no_improve]}"),
...
def train(epoch: int):
    ...
    for i, (data, label, survival_days, age,
            resection) in enumerate(p.track(loader,
                                                task_id=task), 1):
        data, label, survival_days, age, resection = data.to(
            device), label.to(device), survival_days.to(device), age.to(
...
def test(epoch: int):
    ...
    with torch.no_grad():
        ...
        for i, (data, label, survival_days, age,
            ...
            survival_loss = criterion_survival(survival,
                                               survival_days)
            segment_loss = criterion_segment(segment, label)
            loss = survival_loss * 0.75# + segment_loss * 0.25
...
if __name__ == '__main__':
    ...
    with Live(progress_group):
        for op_num, op in enumerate(optimizer_list, 1):
            ...
            model.segformer_encoder.load_state_dict(pretrained_model.segformer_encoder.state_dict())
            model.segformer_decoder.load_state_dict(pretrained_model.segformer_decoder.state_dict())
            ...
            for i, (data, label, survival_days, age,
                ...
                cache[i] = [*[encoded[j].detach().cpu() for j in range(4)], decoded.detach().cpu()]  # 保存到CPU内存
            print(cache.keys())
            task = overall_progress.add_task(f"[{op_num}/{len(optimizer_list)}]Total({op.__name__})", total=epochs, best_epoch=best_epoch, no_improve=no_improve)
            op_folder = experimance_path / op.__name__
            (op_folder / f"model").mkdir(
                parents=True, exist_ok=True)
            optimizer1 = op([
                #{'params': model.segformer_encoder.parameters(), "lr": 2e-06},
                #{'params': model.segformer_decoder.parameters(), "lr": 1e-04},
                {'params': model.decoder_model.parameters(), "lr": 1e-04},
            ...
            for epoch in range(epochs):
                ...
                if len(all_test_acc) <= 1 or valAcc <= min(
                        all_test_acc[:-1]):
                    torch.save(
                        model, op_folder /
                        f"model/best_epoch.pth")
                    torch.save(
                        optimizer1, op_folder /
                        f"model/best_optimizer1.pth")
                    #torch.save(
                    #    optimizer2, experimance_path /
                    #    f"model/best_optimizer2.pth")
                    rprint(f"Best Model Update, Epoch {epoch}")
                    (op_folder /
                    f"best_epoch.txt").write_text(
                        str(epoch))
                    best_epoch = epoch
                    no_improve = 0
                else:
