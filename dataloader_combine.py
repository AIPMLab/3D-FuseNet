...
class BraTSDataset(Dataset):
    ...
    def __init__(
        ...
        label_data = label_data.sort_values(
            "Survival_days")  #[label_data["Survival_days"] > 500]
        ...
        train_data, val_data, test_data = stratified_split_sorted(label_data,
                                                                  "Survival_days",
                                                                  0.7,
                                                                  0.1,
                                                                  0.2,
        ...
        self.clinical = [
            torch.Tensor(
                label_data[label_data["Brats20ID"] == i][["Age", "ER1", "ER2",
                                                          "ER3"]].values[0])
            for i in self.random_index
        ]
        self.images = [
        ...
        self.segment = [
            torch.load(self.data_folder / name / f"{name}_label.pt")
            for name in self.random_index
        ]
        self.rad = [
            torch.Tensor(rad_df[rad_df["name"] == i][coloumn_use_rad].values[0])
            for i in self.random_index
        ]
