...
class RegressionModel(nn.Module):
    ...
    def __init__(self, with_clinical=False, middle=2, clinical_=4) -> None:
        ...
        if with_clinical:
            self.clinical_model = RBFRegression(n_out=clinical_)
        seq_list = [
            nn.Linear(16384, 512),
            nn.Dropout(0.1),
            nn.ReLU(),
...
class RadiomicsExtraction(nn.Module):
...
class CombinationModel(nn.Module):
    ...
    def __init__(self, with_clinical=False, middle=2, clinical_=4) -> None:
        ...
        reversed_embed_dims = [256, 160, 64, 32]
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            fan_out = m.kernel_size[0] * m.kernel_size[1] * m.kernel_size[
                2] * m.out_channels
            fan_out //= m.groups
            m.weight.data.normal_(0, math.sqrt(2.0 / fan_out))
            if m.bias is not None:
    ...
    def freeze_encoder(self):
        for param in self.segformer_encoder.parameters():
    ...
    def forward(self, mri_data, freeze_data=None):
        ...
        if freeze_data is not None:
            ...
            x = self.segformer_encoder(mri_data)
        # # unpacking the embedded features generated by the transformer
        c1 = x[0]
        c2 = x[1]
        c3 = x[2]
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sr_ratio > 1:
            ...
            x_ = self.sr_norm(x_)
            # (batch_size, num_patches, hidden_size)
            kv = (self.key_value(x_).reshape(B, -1, 2, self.num_heads,
                                             self.attention_head_dim).permute(
                                                 2, 0, 3, 1, 4))
            # (2, batch_size, num_heads, num_sequence, attention_head_dim)
        else:
            # (batch_size, num_patches, hidden_size)
            kv = (self.key_value(x).reshape(B, -1, 2, self.num_heads,
                                            self.attention_head_dim).permute(
                                                2, 0, 3, 1, 4))
            # (2, batch_size, num_heads, num_sequence, attention_head_dim)
...
class MixVisionTransformerEx(nn.Module):
    ...
    def __init__(
        ...
        self.embed_5 = PatchEmbedding(
            in_channel=embed_dims[3],
            embed_dim=embed_dims[4],
            kernel_size=patch_kernel_size[4],
            stride=patch_stride[4],
            padding=patch_padding[4],
        )
        ...
        self.norm5 = nn.LayerNorm(embed_dims[4])
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape
        n = cube_root(N)
        for i, blk in enumerate(self.tf_block5):
            x = blk(x)
        x = self.norm5(x)
        # (B, N, C) -> (B, D, H, W, C) -> (B, C, D, H, W)
        x = x.reshape(B, n, n, n, -1).permute(0, 4, 1, 2, 3).contiguous()
