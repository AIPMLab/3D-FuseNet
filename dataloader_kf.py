...
class BraTSDataset(Dataset):
    ...
    def __init__(
        ...
        self.data_folder = Path(
            r"D:\vit_reg\SegFormer3D\data\brats2021_seg\BraTS2020_Training_Data")
        self.size = size
        if kfold < 0:
            train_data = pd.read_csv("data_split/train.csv")
            label_data = pd.read_csv(f"data_split/{data_type}.csv")
        else:
            train_data = pd.read_csv(f"data_split/train_kfold_{kfold}.csv")
            label_data = pd.read_csv(f"data_split/{data_type}_kfold_{kfold}.csv")
        #print(train_data)
        if preprocess:
            ...
            label_data["Age"] = age_scaler.transform(
                label_data["Age"].to_numpy().reshape(-1, 1))
        else:
            self.scaler = None
