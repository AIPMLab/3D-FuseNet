...
train_dataset = BraTSDataset(
    r"D:\yolov9\BraTS2020_TrainingData\MICCAI_BraTS2020_TrainingData", "train", kfold=kf_num)
test_dataset = BraTSDataset(
    r"D:\yolov9\BraTS2020_TrainingData\MICCAI_BraTS2020_TrainingData", "val", kfold=kf_num)
...
if __name__ == '__main__':
    ...
    experimance_path = Path(f"Only_3d_kfold_{kf_num}")
    no_improve = 0
    ...
    for op_num, op in enumerate(optimizer_list, 1):
        ...
        #model.load_state_dict(torch.load("pytorch_model.bin"), strict=False)
        overall_progress.update(task, description=f"[{op_num}/{all_op}]Total({op.__name__})")
        loss = nn.HuberLoss()
